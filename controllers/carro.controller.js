...
export const addProductCarro = async (req, res) => {
  try {
    ...
    if (carroWhitProducts.amount > producto.stock) {
      await carroWhitProducts.update(
      ...
      return res
        .status(201)
        .json({ message: "No hay más productos en stock." });
    }
    ...
    res
      .status(500)
      .json({ code: 500, message: "Error al agregar el producto al carro." });
  }
};
...
export const deleteCarro = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ code: 500, message: "Error al eliminar el carro!." });
  }
};
...
export const deleteProductCarro = async (req, res) => {
  try {
    ...
    if (carroWithProducts === null) {
      return res
        .status(400)
        .json({ message: "El producto que intenta restar no existe." });
    }
    ...
    if (carroWithProducts.dataValues.amount === 0) {
      ...
      return res
        .status(201)
        .json({ message: "Ha quitado todos los productos de este tipo." });
    }
